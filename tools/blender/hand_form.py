...
DIGIT_SPEC = {
    "index":  dict(x=+0.0300, y=+0.0016, z=-0.0975,
                   L=(0.0338, 0.0225, 0.0187), r=(0.0102, 0.0094),
                   splay=+5.5, flex=(7.0, 11.0, 5.0), bury=0.32, backup=0.92),
    "middle": dict(x=+0.0100, y=+0.0020, z=-0.1035,
                   L=(0.0365, 0.0243, 0.0202), r=(0.0102, 0.0096),
                   splay=+0.5, flex=(6.0, 11.0, 5.5), bury=0.22, backup=0.86),
    "ring":   dict(x=-0.0100, y=+0.0018, z=-0.0995,
                   L=(0.0342, 0.0228, 0.0190), r=(0.0096, 0.0090),
                   splay=-5.5, flex=(8.0, 13.0, 6.0), bury=0.20),
    "little": dict(x=-0.0288, y=+0.0010, z=-0.0895,
                   L=(0.0275, 0.0183, 0.0152), r=(0.0084, 0.0079),
                   splay=-12.5, flex=(10.5, 15.0, 7.0), bury=0.26),
    ...
    # The thumb's PALMAR rotation is the knob that was wrong in v3. Bending the
    # thumb PATH 17 deg toward -Y aims it at a palmar camera, and a limb aimed
    # at the camera foreshortens into a stub -- which is exactly what the clay
    # renders showed. A real thumb sits only ~12 deg palmar of the palm plane;
    # what makes it look "turned" is PRONATION about its own long axis, so the
    # nail faces radially. That is `roll`, and it moves the cross-section, not
    # the path.
    "thumb":  dict(x=+0.0170, y=-0.0026, z=-0.0150,
                   L=(0.0380, 0.0320, 0.0258), r=(0.0108, 0.0099),
                   splay=+28.0, flex=(7.0, 13.0, 8.0), palmar=11.0,
                   roll=-78.0, bury=0.30, backup=0.55),
}
...
# The waist/joint amplitude above is a compromise held between two failures.
# v1 waisted to 0.82 and the shafts read as a string of beads; v3 halved that
# to +-3%, which at 0.6 m is below the point where anything reads at all and
# the fingers came out as smooth dowels. +-8% is the most that can be spent
# here without hurting J3: the waist rings are pri 2 and vanish on LOD1/LOD2,
# so every millimetre of waisting is silhouette error the moment the ladder
# steps down.
...
class Hand:
    ...
    def __init__(self, key):
        ...
        self.bumps, self.post = self._bumps()
    ...
    def _bumps(self):
        ...
        Returns TWO lists, and the split is the whole point.

        v3 returned one list, applied entirely before `_lobe`. Everything
        distal of Z_LOBE is then dragged up to 94% of the way onto the four
        digit ellipses, which erases any bump living there -- and the knuckles,
        the metacarpal ridges and the transverse callus pad ALL live there.
        That is why the dorsal clay render came out as a smooth mitten back
        with no knuckles in the silhouette: they were authored, measured, and
        then projected away a few lines later.

        `pre`  - the palm's own mass (thenar, hypothenar, hollow, wrist bones).
                 These are proximal, the lobe blend does not reach them, and
                 they want to be part of the section that gets blended.
        `post` - knuckles, metacarpal ridges, callus pad. Applied AFTER the
                 lobe projection so they survive it and stand off the finished
                 surface, which is what a knuckle physically is: a bone head
                 sitting proud of the web it interrupts.
        """
        s = self.scale
        f = self.f
        B, P = [], []
        ...
        def add_post(a0, z0, amp, wa, wz):
            P.append((a0, z0 * s, amp * s, wa, wz * s))
        ...
        add(+0.86, -0.0175, +0.0098 * f["thenar"], 0.56, 0.0195)
        add(+0.80, -0.0430, +0.0119 * f["thenar"], 0.62, 0.0290)
        ...
        kn = f["knuck"]
        for (dz, dx, amp) in ((-0.0955, +0.0300, 0.00385),
                              (-0.0990, +0.0100, 0.00460),
                              (-0.0965, -0.0100, 0.00415),
                              (-0.0890, -0.0290, 0.00325)):
            a = PI - math.asin(max(-1.0, min(1.0, dx / 0.0436))) * 0.86
            # wa was 0.235 rad. Four knuckles sit ~0.375 rad apart across the
            # dorsum, so a 0.235 skirt overlaps its neighbours badly and the
            # four merge into one smooth transverse ridge -- a knuckle you
            # cannot count is not a knuckle. 0.145 keeps them separate.
            add_post(a, dz, amp * kn, 0.145, 0.0074)
            # metacarpal ridge running back from each knuckle. This one stays
            # PRE: it is proximal of the lobe blend for most of its length and
            # it wants to be part of the palm's mass, not a welt on top of it.
            add(a, dz + 0.0290, 0.00110 * f["tendon"], 0.150, 0.0215)
        # Transverse pad under the finger roots -- the rope callus ridge. POST,
        # and it is the single most important bump on this asset: the painted
        # callus has to sit on real relief or it reads as a stain.
        add_post(+0.02, -0.0845, +0.0040, 0.95, 0.0105)
        return B, P
    ...
    def surface(self, z, a, tuck=1.0):
        ...
        # Post-lobe relief -- knuckles and the callus ridge. See _bumps(): put
        # these before the projection and the projection eats them.
        #
        # Each post bump displaces along ITS OWN fixed azimuth direction
        # (sin a0, -cos a0), not along a per-vertex normal. That matters: the
        # obvious choice, "radially outward from the nearest lobe axis", makes
        # the displacement direction a DISCONTINUOUS function of azimuth,
        # because adjacent columns either side of a lobe boundary pick
        # different axes and push apart. The clay render of that version had a
        # row of triangular shards along the knuckle line -- the same class of
        # bug the v2 author hit keying `lobe_of` on displaced x. A per-bump
        # constant direction is smooth by construction and is also what the
        # anatomy says: a knuckle stands proud DORSALLY, a callus ridge stands
        # proud PALMARLY, neither of them radially from a finger's axis.
        dx_acc = dy_acc = 0.0
        for (a0, z0, amp, wa, wz) in self.post:
            da = (a - a0 + PI) % TAU - PI
            if abs(da) > wa * 2.6 or abs(z - z0) > wz * 2.8:
                continue
            d = amp * math.exp(-(da / wa) ** 2) * math.exp(-((z - z0) / wz) ** 2)
            dx_acc += math.sin(a0) * d
            dy_acc += -math.cos(a0) * d
        x += dx_acc
        y += dy_acc
        return (x, y, z)
    ...
    def _digit_path(self, name):
        ...
        return dict(pts=pts, dirs=dirs, cum=cum, total=total,
                    ...
                    # NOT multiplied by side: char_hands builds every variant as
                    # a right hand and negates X at the end, and that mirror
                    # already reverses the roll.
                    roll=math.radians(sp.get("roll", 0.0)),
