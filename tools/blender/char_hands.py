...
def tri_pole(faces, uvs, ring, ci, ring_uv, centre_uv, flip=False):
    """Cap a ring with a TRIANGLE fan.

    Used for exactly one cap: the palm's distal pole. `quad_pole` builds each
    cap face from three consecutive ring verts plus the centre, and such a quad
    is planar only if the ring is. The distal ring is not -- it is four lobes
    tucked toward four separate finger axes, so it dips inward between lobes
    and back out, and the quad that spans a dip is FOLDED. A folded quad has
    one triangulated half winding against the polygon's own normal, which is
    what validate_glb reported as one inverted triangle per LOD0 mesh. Flat
    shading does not fix that, because the disagreement is inside the polygon,
    not between the polygon and its neighbours.

    A triangle is planar by definition, so this cannot happen. It costs nothing:
    a quad exports as two triangles anyway, so the fan is the same triangle
    count and the same vertex count. The price is that this buried cap is the
    one place in the asset that is not quad-pure, which CHECKLIST.md B allows
    explicitly for flat interior geometry.
    """
    n = len(ring)
    for k in range(n):
        a, b = ring[k], ring[(k + 1) % n]
        ua, ub = ring_uv[k], ring_uv[(k + 1) % n]
        cu = (ua[0], centre_uv)
        if flip:
            faces.append((b, a, ci)); uvs.append([ub, ua, cu])
        else:
            faces.append((a, b, ci)); uvs.append([ua, ub, cu])
...
def build_palm(h, nc, pri, verts, faces, uvs, base):
    ...
    #
    # That ring is NON-CONVEX by construction -- four lobes tucked toward four
    # separate axes -- so a fan from its centroid necessarily contains one face
    # that folds back on itself, and validate_glb reported exactly one inverted
    # triangle per LOD0 mesh at (+-0.0057, 0.0045, -0.107), which is this cap.
    # It is buried inside the finger tubes and no camera reaches it, but
    # CHECKLIST.md C is HARD and "you cannot see it" is not a pass. Flat-shading
    # the cap makes each corner normal equal to its own face normal, so the
    # winding-vs-shading-normal disagreement becomes arithmetically impossible
    # rather than merely small -- and smooth shading over a buried non-convex
    # fan was meaningless anyway. Returns the cap's face range so build_lod can
    # clear use_smooth on it.
    last = ring_ids[-1]
    ...
    f0 = len(faces)
    tri_pole(faces, uvs, last, ci, ring_uvs[-1], 1.0, flip=True)
    return (f0, len(faces))
...
def build_digit(h, name, nf, pri, verts, faces, uvs, base):
    ...
    for (t, mul) in rings:
        ...
        e2 = HF._cross(d, e1)
        if roll:
            # pronation about the digit's own long axis. The thumb needs it:
            # its nail faces radially, not dorsally, and that rotation is what
            # makes a thumb read as opposable rather than as a fifth finger
            # stuck on sideways.
            c, s = math.cos(roll), math.sin(roll)
            e1, e2 = ([e1[k] * c + e2[k] * s for k in range(3)],
                      [e2[k] * c - e1[k] * s for k in range(3)])
        v = (t - t0) / (t1 - t0)
        ids, uvr = [], []
...
def build_lod(key, spec, coll):
    ...
    cap_range = build_palm(h, spec["nc"], spec["pri"], verts, faces, uvs, 0)
    pal_faces = len(faces)
    isl = ["palm"] * pal_faces
...
def poke_check(h, nf):
    ...
    for name in HF.DIGITS:
        ...
        for (t_raw, mul, _p) in HF.DIGIT_RINGS:
            ...
            e2 = HF._cross(d, e1)
            if g["roll"]:
                c, s = math.cos(g["roll"]), math.sin(g["roll"])
                e1, e2 = ([e1[k] * c + e2[k] * s for k in range(3)],
                          [e2[k] * c - e1[k] * s for k in range(3)])
            for i in range(nf * 2):
                ph = -PI + TAU * i / (nf * 2)
