...
def paint_palm(key, h, p, W, Hh, seed):
    ...
    base = mix(base, ccol, np.clip(callus * 0.92, 0, 1))
    rough += np.clip(callus, 0, 1) * (ROUGH_CALLUS - ROUGH_SKIN)
    hgt += np.clip(callus, 0, 1) * 1.15
...
def paint_digit(name, key, h, W, Hh, seed):
    ...
    croot = band(V, v_mcp + 0.055, 0.030, 0.040) * pad * f["callus"] * wear
    ccol = mix(CALLUS, CALLUS_OLD, min(1.0, max(0.0, (h.age - 19) / 41.0)))
    base = mix(base, ccol, np.clip(croot * 0.80, 0, 1))
    rough += np.clip(croot, 0, 1) * (ROUGH_CALLUS - ROUGH_SKIN)
    hgt += np.clip(croot, 0, 1) * 0.95
